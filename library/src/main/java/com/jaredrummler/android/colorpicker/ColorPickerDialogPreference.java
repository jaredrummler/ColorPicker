 ...
 * </pre>
 */
public class ColorPickerDialogPreference extends PreferenceDialogFragmentCompat implements OnTouchListener,
    ...
    static final String ARG_TYPE = "dialogType";
    static final String ARG_COLOR = "color";
    static final String ARG_ALPHA = "alpha";
    static final String ARG_PRESETS = "presets";
    static final String ARG_ALLOW_PRESETS = "allowPresets";
    static final String ARG_ALLOW_CUSTOM = "allowCustom";
    static final String ARG_DIALOG_TITLE = "dialogTitle";
    static final String ARG_SHOW_COLOR_SHADES = "showColorShades";
    static final String ARG_COLOR_SHAPE = "colorShape";
    static final String ARG_PRESETS_BUTTON_TEXT = "presetsButtonText";
    static final String ARG_CUSTOM_BUTTON_TEXT = "customButtonText";
    static final String ARG_SELECTED_BUTTON_TEXT = "selectedButtonText";
    ...
    public ColorPickerDialogPreference() {
    ...
    public void onAttach(Context context) {
        ...
        if (getTargetFragment() instanceof ColorPickerDialogListener) {
    ...
    public Dialog onCreateDialog(Bundle savedInstanceState) {
        dialogId = getColorPreference().getArgs().getInt(ARG_ID);
        showAlphaSlider = getColorPreference().getArgs().getBoolean(ARG_ALPHA);
        showColorShades = getColorPreference().getArgs().getBoolean(ARG_SHOW_COLOR_SHADES);
        colorShape = getColorPreference().getArgs().getInt(ARG_COLOR_SHAPE);
        if (savedInstanceState == null) {
            color = getColorPreference().getColor();
            dialogType = getColorPreference().getArgs().getInt(ARG_TYPE);
        } else {
        ...
        int selectedButtonStringRes = getColorPreference().getArgs().getInt(ARG_SELECTED_BUTTON_TEXT);
        ...
        int dialogTitleStringRes = getColorPreference().getArgs().getInt(ARG_DIALOG_TITLE);
        if (dialogTitleStringRes != 0) {
            builder.setTitle(dialogTitleStringRes);
        }

        presetsButtonStringRes = getColorPreference().getArgs().getInt(ARG_PRESETS_BUTTON_TEXT);
        customButtonStringRes = getColorPreference().getArgs().getInt(ARG_CUSTOM_BUTTON_TEXT);

        int neutralButtonStringRes;
        if (dialogType == TYPE_CUSTOM && getColorPreference().getArgs().getBoolean(ARG_ALLOW_PRESETS)) {
            neutralButtonStringRes =
                    (presetsButtonStringRes != 0 ? presetsButtonStringRes : R.string.cpv_presets);
        } else if (dialogType == TYPE_PRESETS && getColorPreference().getArgs().getBoolean(ARG_ALLOW_CUSTOM)) {
            neutralButtonStringRes =
    ...
    private ColorPreferenceSupport getColorPreference() {
        return (ColorPreferenceSupport) getPreference();
    }
    ...
    private void onColorSelected() {
        ColorPreferenceSupport preference = getColorPreference();
        if (preference.callChangeListener(color)) {
            preference.onColorSelected(dialogId, color);
        }
    }

    private void loadPresets() {
        int alpha = Color.alpha(color);
        presets = getColorPreference().getArgs().getIntArray(ARG_PRESETS);
            ...
            colorPanelView.setOnClickListener(new View.OnClickListener() {
                ...
                public void onClick(View v) {
                    if (v.getTag() instanceof Boolean && (Boolean) v.getTag()) {
                        colorPickerDialogListener.onColorSelected(dialogId, ColorPickerDialogPreference.this.color);
                        dismiss();
                        return; // already selected
                    }
                    ColorPickerDialogPreference.this.color = colorPanelView.getColor();
                    adapter.selectNone();
                    for (int i = 0; i < shadesLayout.getChildCount(); i++) {
                        FrameLayout layout = (FrameLayout) shadesLayout.getChildAt(i);
